...
export default function CheckoutPage() {
  ...
  if (!meal && !orderComplete) {
    return (
      <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center">
        ...
        <h2 className="text-[#fafafa] font-black uppercase tracking-widest text-[10px]">Verifying Meal Details...</h2>
      </div>
    );
  }
  ...
  if (orderComplete) {
    return (
      <div className="min-h-screen bg-[#050505] flex items-center justify-center p-6">
        <div className="bg-[#0d0d0d] border border-white/5 p-10 rounded-[3rem] text-center max-w-sm w-full shadow-2xl flex flex-col items-center">
          ...
          <h2 className="text-3xl font-black uppercase italic text-[#fafafa] mb-8 tracking-tighter">Verified</h2>
          ...
          <button onClick={() => router.push("/studentMeals")} className="w-full bg-white text-black py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-orange-500 hover:text-[#fafafa] transition-all">Done</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#050505] text-[#fafafa] pt-24 pb-20 px-4 md:px-10">
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="reveal-stack">
          <button onClick={() => router.back()} className="flex items-center gap-2 text-gray-500 hover:text-[#fafafa] transition-colors uppercase text-[10px] font-black tracking-widest"><ArrowLeft size={16} /> Back</button>
        </div>
        ...
        <div className="reveal-stack bg-[#0d0d0d] border border-white/5 p-8 rounded-[2.5rem] space-y-8 shadow-2xl">
          <div className="space-y-4">
            ...
            <div onClick={() => setIsBitePaySelected(true)} className={`p-6 rounded-[2rem] border-2 transition-all cursor-pointer flex items-center justify-between ${isBitePaySelected ? 'border-orange-500 bg-orange-500/5' : 'border-white/5 bg-black/40'}`}>
              <div className="flex items-center gap-4">
                <div className={`p-3 rounded-xl ${isBitePaySelected ? 'bg-orange-500 text-[#fafafa]' : 'bg-white/5 text-gray-500'}`}><CreditCard size={24} /></div>
                <span className="font-black uppercase text-sm tracking-tight">BitePay Wallet</span>
              </div>
              <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${isBitePaySelected ? 'border-orange-500' : 'border-white/5'}`}>{isBitePaySelected && <div className="w-2.5 h-2.5 bg-orange-500 rounded-full" />}</div>
            </div>
          </div>
          <div className={`space-y-4 transition-all duration-500 ${isBitePaySelected ? 'opacity-100' : 'opacity-20 pointer-events-none'}`}>
            ...
            <input type="password" maxLength={4} value={securityPin} onChange={(e) => setSecurityPin(e.target.value)} className="w-full bg-black border border-white/5 p-6 rounded-2xl text-center text-4xl font-black tracking-[0.8em] focus:border-orange-500 outline-none transition-all" placeholder="****" />
          </div>
