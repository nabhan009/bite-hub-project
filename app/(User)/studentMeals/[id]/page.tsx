...
import { ArrowLeft, MapPin, Clock, Info, ShoppingCart, CalendarCheck } from "lucide-react";
...
useLayoutEffect(() => {
  ...
  if (!meal) return <div className="h-screen bg-[#050505] flex items-center justify-center text-orange-500 font-black italic">LOADING...</div>;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="animate-up relative aspect-square lg:aspect-auto lg:h-[70vh] rounded-[3rem] overflow-hidden border border-white/10 shadow-2xl">
            ...
            <div className="absolute bottom-10 left-10">
               <span className={`${meal.type === 'veg' ? 'bg-green-600' : 'bg-red-600'} px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl`}>
                 {meal.type}
               </span>
            </div>
          </div>
          ...
          <div className="space-y-8">
            ...
            <div className="animate-up">
              <h3 className="text-gray-500 text-xs font-black uppercase tracking-[0.4em] mb-3">{meal.hotelName}</h3>
              <h1 className="text-6xl lg:text-8xl font-black uppercase tracking-tighter leading-none">
                {meal.foodName.split(' ')[0]} <br />
                <span className="text-orange-500 italic">{meal.foodName.split(' ').slice(1).join(' ')}</span>
              </h1>
              <p className="text-4xl font-black mt-6 text-white tracking-tight">₹{meal.price}</p>
            </div>
            ...
            <div className="animate-up space-y-4 pt-4 border-t border-white/5">
              ...
              <p className="text-gray-400 text-lg leading-relaxed max-w-xl">
                {meal.description || "A masterfully crafted student meal prepared with fresh ingredients. High protein, balanced nutrition, and delivered straight from the kitchen of " + meal.hotelName + "."}
              </p>
            </div>

            {/* BUTTON ACTIONS */}
            <div className="animate-up flex flex-col sm:flex-row gap-4 pt-8">
              <button className="flex-1 bg-orange-600 hover:bg-orange-500 text-white py-6 rounded-[1.5rem] font-black uppercase text-xs tracking-widest flex items-center justify-center gap-3 transition-all shadow-xl shadow-orange-600/20 active:scale-95">
                <ShoppingCart size={18} /> Buy Now
              </button>
              <button className="flex-1 border border-white/20 hover:bg-white hover:text-black text-white py-6 rounded-[1.5rem] font-black uppercase text-xs tracking-widest flex items-center justify-center gap-3 transition-all active:scale-95">
