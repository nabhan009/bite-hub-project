      ...
      <main className="flex-1 ml-16 md:ml-20 p-4 md:p-10 overflow-x-hidden overflow-y-auto">
        <header className="mb-12">
          <h1 className="text-4xl font-black uppercase tracking-tighter italic">
            Partner <span className="text-orange-500">Network</span>
          </h1>
          <p className="text-gray-500 text-[10px] md:text-xs font-bold uppercase mt-2 tracking-widest">
            Kozhikode Restaurant Audit & Control
          </p>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {hotels.map((h) => (
            <div
              key={h.id}
              className="bg-[#0d0d0d] border border-white/5 p-8 rounded-[2.5rem] relative overflow-hidden group hover:border-orange-500/30 transition-all"
            >
              <div
                className={`absolute top-0 right-0 w-1 h-full ${h.isBlocked ? "bg-red-500" : "bg-green-500"}`}
              />
              ...
              <div className="flex justify-between items-start mb-6">
                ...
                <span
                  className={`text-[9px] font-black uppercase tracking-widest ${h.isBlocked ? "text-red-500" : "text-green-500"}`}
                >
              ...
              <p className="text-gray-500 text-[10px] font-bold uppercase flex items-center gap-1 mb-8">
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={() => {
                    toast("Remove this hotel?", {
                      description: "This action cannot be undone.",
                      action: {
                        label: "Delete",
                        onClick: async () => {
                          await api.delete(`/restaurants/${h.id}`);
                          fetchHotels();
