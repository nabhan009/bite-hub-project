...
import { LayoutDashboard, History, PlusCircle, UserCircle } from "lucide-react";
  ...
  return (
    <nav className="fixed top-4 md:top-8 left-0 right-0 z-[100] flex items-center justify-center gap-2 md:gap-4 px-2">
      ...
      <div className="relative bg-[#0d0d0d]/80 backdrop-blur-xl border border-white/5 p-1 md:p-1.5 rounded-full flex items-center shadow-2xl">
        ...
        {navLinks.map((link, i) => (
          <Link
            ...
            className={`relative z-10 px-4 md:w-[110px] py-2 flex flex-col items-center justify-center transition-all duration-300 ${
              pathname === link.path ? "text-orange-500" : "text-gray-500 hover:text-white"
            }`}
          >
      ...
      <div 
        ...
        className="bg-[#0d0d0d]/90 backdrop-blur-md border border-white/10 h-10 md:h-12 px-2 md:px-3 rounded-xl md:rounded-2xl flex items-center shadow-xl cursor-pointer group"
      >
        <div className="flex flex-col items-end mr-3 hidden sm:block">
          <span className="text-[9px] font-black uppercase tracking-tighter text-white group-hover:text-orange-500 transition-colors">
