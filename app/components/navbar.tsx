...
export default function NavbarPersonalized() {
  ...
  return (
    <nav className="fixed top-4 md:top-8 left-0 right-0 z-[100] flex items-center justify-center gap-2 md:gap-4 px-2">
      ...
      <Link href="/studentHome" className="bg-[#0d0d0d]/80 backdrop-blur-md border border-white/10 w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl flex items-center justify-center shadow-xl group transition-all shrink-0">
      ...
      <div className="relative bg-[#0d0d0d]/80 backdrop-blur-xl border border-white/5 p-1 md:p-1.5 rounded-full flex items-center shadow-2xl">
        ...
        {navLinks.map((link, i) => (
          <Link
            ...
            onMouseEnter={() => setHoveredIndex(i)}
            onMouseLeave={() => setHoveredIndex(null)}
            className={`relative z-10 px-3 md:w-[100px] py-1.5 md:py-2 text-center text-[9px] md:text-[10px] italic font-black uppercase tracking-widest transition-colors duration-300 ${
              pathname === link.path ? "text-orange-500" : "text-gray-400 hover:text-white"
            }`}
          >
            {link.name}
      ...
      <div 
        ...
        onMouseEnter={() => handleProfileHover(true)}
        onMouseLeave={() => handleProfileHover(false)}
        className="bg-[#0d0d0d]/80 backdrop-blur-md border border-white/10 h-10 md:h-12 px-1.5 md:px-2 rounded-xl md:rounded-2xl flex items-center shadow-xl cursor-pointer shrink-0 transition-all"
      >
        <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg md:rounded-xl bg-orange-500 flex items-center justify-center text-[10px] font-black text-white shadow-lg shadow-orange-500/20">
        ...
        <span 
          ref={nameRef}
          className="overflow-hidden whitespace-nowrap opacity-0 w-0 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-gray-200"
        >
